...
namespace ECTool.Scripts.EditorTools
{
    ...
    public class GrassInspector : CustomInspector
    {
        ...
        void ShowObject(int index, SerializedProperty element, SerializedProperty list, float depth) {
            ...
            // this shows all the children
            // get the parent game object
            // get all the other game objects
            
            EditorGUILayout.BeginHorizontal();
             switch (element.objectReferenceValue)
                {
                ...
                var buttonDefaultWidth = (currentWidth / 100) * 70 - depth;
                ...
                else
                {
                    ...
                    if (GUILayout.Button(name.ToString(), "Button", GUILayout.Height(18),
                    ...
                    if (GUILayout.Button("Delete", GUILayout.Height(18), GUILayout.Width(deleteWidth)))
                    {
                        // need to find the related game objects and destroy them as well? or to just rebuild?
                ...
                for (int i = 0; i < list.arraySize; i++)
                {
                    ...
                    {
                        case MeshGrassSO meshGrassSo:
                            if ( meshGrassSo.Object.go == m_ruleset.GetGameObjectFromProperty(property))
                            {
                                for (int j = i; j >= 0; j--)
                                {
                                    SerializedProperty prevProperty = list.GetArrayElementAtIndex(i);
                                    if (meshGrassSo.Object.go == m_ruleset.GetGameObjectFromProperty(prevProperty))
                                    {
                                ...
                                ShowObject(i, property, list, thisDepth + 20);
                            }
    
                            break;
                        case GrassCardSO enemyObject:
                            if ( enemyObject.Object.go == m_ruleset.GetGameObjectFromProperty(property))
                            {
                                for (int j = i; j >= 0; j--)
                                {
                                    SerializedProperty prevProperty = list.GetArrayElementAtIndex(i);
                                    if (enemyObject.Object.go == m_ruleset.GetGameObjectFromProperty(prevProperty))
                                    {
                                        break;
                                    }
                                }
